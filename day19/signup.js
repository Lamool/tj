...
function signup () {
    ...
    // 1. 아이디와 비밀번호는 5글자 이상만 가능
    if (id.length < 5) {
        alert('회원가입 실패 : 아이디는 5글자 이상 입력해주세요.');
        return;
    }

    if (pw.length < 5) {
        alert('회원가입 실패 : 비밀번호는 5글자 이상 입력해주세요.');       
        return;
    }
    ...
    // 2. 이름은 3글자 이상만 가능
    if (name.length < 3) {
        alert('회원가입 실패 : 이름은 3글자 이상 입력해주세요.');
        return;
    }
    ...
    if (phone.length < 8 || isNaN(phone)) {
        alert('회원가입실패 : 연락처는 - 제외한 8글자 이상 입력해주세요.');
        return;
    }
    ...
    // 4. 비밀번호와 비밀번호 확인 일치 했을 때만 가능
    if (pw != pwconfirm) {
        alert('회원가입실패 : 두 비밀번호가 일치하지 않습니다.');
        return;
    }


    // 5. 아이디와 연락처는 중복 제외
    // 아이디 중복 검사 : 회원목록내 i번째 회원 아이디와 입력받은 아이디가 같으면
    for (let i = 0; i < memberList.length; i++) {
        if (memberList[i].id == id) {
            alert('회원가입실패 : 사용중인 아이디입니다.');
            return;
        }
    }

    // 연락처 중복 검사 : 회원목록내 i번째 회원 연락처와 입력받은 연락처가 같으면
    for (let member of memberList) {
        if(member.phone == phone) {
            alert('회원가입실패 : 전화번호가 중복입니다.');
            return;
        }
    }
    ...
    // 3. 데이터 가공
    // 만약에 회원목록에 회원이 없으면 1 있으면 마지막 회원의 번호
