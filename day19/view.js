/*
    ...
    게시물 상세 페이지
        - 전체 출력 페이지에서 특정 제목 클릭시 <a> 이용한 상세페이지 이동
...
// 1. new URL(URL) : URL(주소)의 정보 호출, location.href : 현재 URL
...
function _delete() {
    ...
    // 삭제
    boardList.splice(findBoardIndex, 1);         // JS 배열 내 객체 삭제
...
// 4. 수정페이지로 이동 함수 : 수정버튼 클릭시
// - 로그인된 회원 - 게시물 작성자 일치 여부
function modify() {

    // 1. 누구를 : 현재 페이지의 게시물번호 = no
    // 2. 해당 수정할 게시물번호의 인덱스 찾기
    let findBoardIndex = -1;
    for (let i = 0; i < boardList.length; i++) {
        if(boardList[i].no == no) {
            findBoardIndex = i;
