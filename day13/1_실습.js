...
// 4. 수정함수 실행조건 : (누구를)1. 수정 버튼 클릭 할 때
function _update(index){
    ...
    let board = boardList[index].split(',');
        // 새로운 제목과 내용 입력받는다.
    let utitle = propmpt('수정할 제목');
    let utcontent = propmpt('수정할 내용');
        // 수정된 정보로 구성 : 새로운 제목과 내용만 수정 변수로 보고 나머지는 기존 데이터 사용
    let 
}
...
function _delete(index){
    ...
    // 2. 해당 인덱스의 비밀번호와 입력받은 비밀번호가 다르면 함수를 장군에 
